...
public class UsuarioServiceImpl implements UsuarioService {
    ...
    public ResponseEntity<String> activarUsuario(String email, String codigo) throws Exception {
        Usuario user = usuarioDao.findOneByEmail(email)
                .orElseThrow(() -> new NoSuchElementException("No existe cuenta registrada con el email ingresado"));
        if(user.getActivo()) throw new UsuarioRegistradoException("La cuenta ya ha sido activada");
        ...
        return ResponseEntity
                .badRequest()
                .body("El código ingresado no es válido o ha expirado. Hemos enviado un nuevo código a tu email");
    }
